import { initializeApp, getApps } from "https://www.gstatic.com/firebasejs/10.8.0/firebase-app.js";
import {
  getAuth,
  GoogleAuthProvider,
  onAuthStateChanged,
  signInWithCredential,
  signInWithEmailAndPassword,
  signOut
} from "https://www.gstatic.com/firebasejs/10.8.0/firebase-auth.js";
import {
  doc,
  getDoc,
  getFirestore,
  serverTimestamp,
  setDoc
} from "https://www.gstatic.com/firebasejs/10.8.0/firebase-firestore.js";

const config = window.ADNN_FIREBASE_CONFIG;
const app = config ? (getApps()[0] || initializeApp(config)) : null;
const auth = app ? getAuth(app) : null;
const db = app ? getFirestore(app) : null;

syncStoredGoogleAccountToFirebase();
window.adnnSyncGoogleAccountFirebase = syncStoredGoogleAccountToFirebase;

if (auth && db && location.pathname.includes("designer-account.html")) {
  onAuthStateChanged(auth, async (user) => {
    if (!user?.uid || !isPasswordUser(user)) return;
    const snap = await getDoc(doc(db, "designers", user.uid)).catch(() => null);
    if (!snap?.exists()) return;
    const designer = snap.data() || {};
    const payload = {
      uid: user.uid,
      name: designer.name || user.email || "Designer",
      email: user.email || designer.authEmail || designer.email || "",
      authEmail: user.email || designer.authEmail || "",
      role: "designer",
      designerid: designer.designerId || designer.designerid || ""
    };
    localStorage.setItem("adnnDesignerUser", JSON.stringify(payload));
    if (typeof window.hydrateUser === "function") window.hydrateUser();
  });
}

function cleanId(value) {
  return String(value || "").trim().toUpperCase();
}

function emailKey(value) {
  return String(value || "").trim().toLowerCase();
}

function isPasswordUser(user) {
  return Boolean(user?.providerData?.some((providerData) => providerData.providerId === "password"));
}

function getStoredGoogleProfile() {
  try {
    const data = JSON.parse(localStorage.getItem("adhnanPortfolioUser") || "null");
    return data && typeof data === "object" ? data : null;
  } catch (_) {
    return null;
  }
}

function getStoredGoogleToken() {
  try {
    return sessionStorage.getItem("adnnGoogleAccessToken") || "";
  } catch (_) {
    return "";
  }
}

async function writeClientProfile(user, profile = {}) {
  if (!db || !user?.uid) return;
  const email = emailKey(user.email || profile.email);
  const name = profile.name || user.displayName || email.split("@")[0] || "Client";
  await setDoc(doc(db, "clients", user.uid), {
    uid: user.uid,
    email,
    authEmail: email,
    displayEmail: email,
    emailKeys: Array.from(new Set([email, profile.email].filter(Boolean).map(emailKey))),
    name,
    displayName: name,
    photoURL: user.photoURL || profile.picture || profile.photoURL || "",
    picture: profile.picture || user.photoURL || "",
    role: email === "getavcollab@gmail.com" ? "admin" : "client",
    provider: "google",
    lastActiveAt: serverTimestamp(),
    updatedAt: serverTimestamp(),
    updatedAtMs: Date.now()
  }, { merge: true });
}

async function syncStoredGoogleAccountToFirebase() {
  if (!auth || !db) return null;
  const accessToken = getStoredGoogleToken();
  const profile = getStoredGoogleProfile();
  const profileEmail = emailKey(profile?.email);
  if (!accessToken || !profileEmail) return null;

  try {
    if (auth.currentUser && emailKey(auth.currentUser.email) === profileEmail) {
      await writeClientProfile(auth.currentUser, profile);
      return auth.currentUser;
    }

    const credential = GoogleAuthProvider.credential(null, accessToken);
    const result = await signInWithCredential(auth, credential);
    await writeClientProfile(result.user, profile);
    window.dispatchEvent(new CustomEvent("adnnFirebaseClientReady", {
      detail: {
        uid: result.user.uid,
        email: result.user.email || profileEmail,
        name: profile.name || result.user.displayName || profileEmail,
        role: profileEmail === "getavcollab@gmail.com" ? "admin" : "client"
      }
    }));
    return result.user;
  } catch (error) {
    console.warn("AdnnStudio Firebase Google session sync failed", error);
    return null;
  }
}

window.signInDesignerWithFirebase = async function signInDesignerWithFirebase(designer, password) {
  if (!auth || !db) {
    throw new Error("Firebase is not connected.");
  }

  const authEmail = String(designer.authEmail || designer.email || "").trim().toLowerCase();
  if (!authEmail) throw new Error("Designer auth email is missing.");

  const credential = await signInWithEmailAndPassword(auth, authEmail, password);
  const user = credential.user;
  const designerId = cleanId(designer.designerid || designer.designerId);
  const displayEmail = user.email || designer.authEmail || designer.email || "";
  const name = designer.name || `Designer ${designerId}`;

  await setDoc(doc(db, "designers", user.uid), {
    uid: user.uid,
    designerId,
    designerid: designerId,
    designerIdLower: String(designerId || "").toLowerCase(),
    authEmail: user.email || authEmail,
    email: displayEmail,
    emailKeys: Array.from(new Set([user.email, authEmail, displayEmail].filter(Boolean).map((mail) => String(mail).trim().toLowerCase()))),
    uidAliases: Array.from(new Set([user.uid, designerId, String(designerId || "").toLowerCase()].filter(Boolean))),
    name,
    role: "designer",
    lastActiveAt: serverTimestamp()
  }, { merge: true });

  const payload = {
    uid: user.uid,
    name,
    email: displayEmail,
    authEmail: user.email || authEmail,
    role: "designer",
    designerid: designerId
  };
  window.dispatchEvent(new CustomEvent("adnnDesignerFirebaseReady", { detail: payload }));
  return payload;
};

window.signOutDesignerFirebase = async function signOutDesignerFirebase() {
  if (auth) await signOut(auth).catch(() => {});
};
