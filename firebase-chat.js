  ...
  shell.innerHTML = `
    ...
    <main class="adnn-room" data-room>
      ...
      <form class="adnn-composer" data-composer>
        ...
        <button class="adnn-action primary" type="submit">${I.send}</button>
      </form>
    </main>`;
...
function bindComposer(shell, chat) {
  const form = shell.querySelector("[data-composer]"); const text = shell.querySelector("[data-text]"); const file = shell.querySelector("[data-file]"); const pick = shell.querySelector("[data-pick]"); const rec = shell.querySelector("[data-record]"); const prev = shell.querySelector("[data-attach-preview]");
  pick.onclick = () => file.click();
  file.onchange = () => { prev.classList.toggle("show", !!file.files[0]); shell.querySelector("[data-attach-name]").textContent = file.files[0]?.name || ""; };
  shell.querySelector("[data-attach-clear]").onclick = () => { file.value = ""; prev.classList.remove("show"); };
  ...
  form.onsubmit = async e => { e.preventDefault(); await sendMessage(chat, text.value.trim(), file.files[0]); text.value=""; text.style.height=""; file.value=""; prev.classList.remove("show"); };
  rec.onclick = () => toggleRecording(chat, rec);
}

async function sendMessage(chat, text, file) {
  if (!text && !file) return;
  let media = {};
  try {
    if (file) {
      toast("Uploading attachment...");
      const path = `chat-media/${chat.id}/${user.uid}/${Date.now()}-${uidSafe(file.name)}`;
      const r = storageRef(storage, path); await uploadBytes(r, file); media.mediaUrl = await getDownloadURL(r); media.mediaType = file.type || "application/octet-stream"; media.mediaName = file.name;
    }
    const data = { senderUid:user.uid, senderEmail:user.email || "", senderName:displayName(user), text:text || "", ...media, createdAt:serverTimestamp(), reactions:{}, status:"sent" };
    await addDoc(collection(db, "chats", chat.id, "messages"), data);
    await setDoc(doc(db, "chats", chat.id), { lastMessage:text || (file ? `Attachment: ${file.name}` : "Message"), lastSenderUid:user.uid, updatedAt:serverTimestamp() }, { merge:true });
  } catch (e) { console.error(e); toast("Could not send. Check Firestore/Storage rules and sign-in."); }
}

async function toggleRecording(chat, button) {
  if (recorder && recorder.state === "recording") { recorder.stop(); button.classList.remove("danger"); return; }
  try {
    ...
    recorder.onstop = async () => { stream.getTracks().forEach(t=>t.stop()); const blob = new Blob(chunks, { type:"audio/webm" }); const file = new File([blob], `voice-${Date.now()}.webm`, { type:"audio/webm" }); await sendMessage(chat, "Voice message", file); };
    recorder.start(); button.classList.add("danger"); toast("Recording voice message. Tap mic again to send.");
