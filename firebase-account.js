...
async function syncClientDoc(user) {
  ...
  if (!isAdminEmail(user.email) && isInactiveClient(existingData)) {
...
async function firebaseGoogleLogin() {
  ...
  try {
    ...
    if (location.pathname.endsWith("index.html") || location.pathname === "/" || location.pathname === "") {
      location.href = isAdminEmail(result.user.email) ? "admin.html" : "account.html#notifications";
    }
  } catch (error) {
...
if (auth) {
  // Bridge custom Google OAuth token into Firebase Auth silently
  const token = sessionStorage.getItem("adnnGoogleAccessToken");
  if (token && !auth.currentUser) {
    const credential = GoogleAuthProvider.credential(null, token);
    signInWithCredential(auth, credential).catch((error) => {
      console.warn("AdnnStudio silent Firebase auth failed", error);
    });
  }
...
function startClientStatusListener(user) {
  if (!db || !user?.uid || isAdminEmail(user.email)) return;
  if (clientStatusUnsubscribe) clientStatusUnsubscribe();
